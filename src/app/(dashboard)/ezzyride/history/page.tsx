...
export default function PassengerHistoryPage() {
  ...
  useEffect(() => {
    if (!profile) return;
    supabase.from('rides')
      .select('*, rider:rider_profiles!rides_rider_id_fkey(profiles(full_name))')
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <Link href="/ezzyride" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-display font-black text-3xl">My Trips</h1>
          <p className="text-muted-foreground text-sm">Past rides and deliveries</p>
        </div>
      </div>
            ...
            {rides.map((ride) => (
              <div key={ride.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 glass border border-white/5 rounded-2xl hover:border-white/10 transition-colors gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Rider: {ride.rider?.profiles?.full_name || 'N/A'}</p>
                </div>
